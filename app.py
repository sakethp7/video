...
# # Function to directly render Manim code within Streamlit
# def render_manim_locally(manim_code: ManimCodeOutput):
#     try:
...
#             scene_file = os.path.join(temp_dir, f"scene_{render_id}.py")
#             with open(scene_file, "w", encoding="utf-8") as f:
#                 f.write(manim_code.code)
            
#             # Create a runner script that imports and runs the scene
#             runner_file = os.path.join(temp_dir, f"runner_{render_id}.py")
#             with open(runner_file, "w", encoding="utf-8") as f:
#                 # Create a script that configures and runs Manim
#                 f.write(f"""
# import os
# import sys
# from manim import *
...
# scene_module = __import__("scene_{render_id}")

# # Configure Manim
# config.media_dir = r"{media_dir}"
...
#             # Check various places where Manim might output the video
#             possible_locations = [
#                 # Look in standard quality folders
#                 os.path.join(media_dir, "videos", f"scene_{render_id}", "1080p60"),
#                 os.path.join(media_dir, "videos", f"scene_{render_id}", "720p30"),
#                 os.path.join(media_dir, "videos", f"scene_{render_id}", "480p15"),
#                 # Also check directly in videos folder
#                 os.path.join(media_dir, "videos", f"scene_{render_id}"),
#                 os.path.join(media_dir, "videos"),
#             ]
            
#             for location in possible_locations:
#                 if os.path.exists(location):
#                     video_files = glob.glob(os.path.join(location, "*.mp4"))
#                     if video_files:
#                         video_file = max(video_files, key=os.path.getmtime)
#                         break
            
#             if not video_file:
#                 # If video file not found, search the entire media directory
#                 for root, _, files in os.walk(media_dir):
#                     for file in files:
#                         if file.endswith(".mp4"):
#                             video_path = os.path.join(root, file)
#                             if not video_file or os.path.getmtime(video_path) > os.path.getmtime(video_file):
#                                 video_file = video_path
            
#             if video_file:
#                 # Create a folder to store rendered videos in the Streamlit app directory
#                 output_dir = os.path.join(os.getcwd(), "rendered_videos")
#                 os.makedirs(output_dir, exist_ok=True)
                
#                 # Copy the video to the output directory with a unique name
#                 output_path = os.path.join(output_dir, f"math_animation_{render_id}.mp4")
#                 shutil.copy2(video_file, output_path)
                
#                 return {"success": True, "video_path": output_path}
#             else:
#                 return {"success": False, "error": "No video file was produced by Manim"}
...
#                 # Animate rendering progress while rendering is happening
#                 rendering_thread = st.empty()
                
#                 # Actually render the Manim code (this might take a while)
#                 rendering_result = render_manim_locally(manim_code)
                
#                 if rendering_result["success"]:
#                     st.session_state.video_path = rendering_result["video_path"]
#                     rendering_progress.progress(100)
#                     rendering_status.markdown("✅ Video rendered successfully!")
#                     st.session_state.generation_complete = True
#                 else:
...
#     # Display results after generation is complete
#     if st.session_state.generation_complete and st.session_state.video_path:
#         st.markdown("### 🎉 Your Math Animation is Ready!")
        
#         # Display the video directly from the local file
